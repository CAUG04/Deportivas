"""nfl/train.py: wires the shared moneyline pipeline to nfl_v1 with the
...
def test_wires_sport_feature_set_and_prefix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("deportivas.models.nfl.train.compute_and_write_moneyline_model", _fake)

    result = compute_and_write_nfl_moneyline_model("usa-nfl", calibration_method="platt")

    assert result == [10, 20]
    assert calls["competition_id"] == "usa-nfl"
    assert calls["sport"] == "american_football"
    assert calls["feature_set"] == "nfl_v1"
    assert calls["model_name_prefix"] == "nfl_moneyline"
